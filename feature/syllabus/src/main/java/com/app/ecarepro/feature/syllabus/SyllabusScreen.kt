...
private fun SyllabusScreenContent(
    ...
    LaunchedEffect(pagerState.currentPage, pagerState.isScrollInProgress) {
        if (!pagerState.isScrollInProgress) {
            handleIntent(SyllabusIntent.OnClassSelected(pagerState.currentPage))
        }
    }
    ...
    EcareProScaffold(
        topBar = {
            Column {
                EcareProTopAppBar(
                    title = stringResource(R.string.feature_syllabus_title),
                    ...
                    actions = {
                        TextButton(onClick = { handleIntent(SyllabusIntent.OnAddNewClicked) }) {
                            Icon(
                                ...
                                contentDescription = stringResource(R.string.feature_syllabus_add_new),
                                tint = MaterialTheme.appColors.primary
                            )
                            Text(
                                text = stringResource(R.string.feature_syllabus_add_new),
                    ...
                    EcareProClassTabs(
                        ...
                        onTabClick = {
                            coroutineScope.launch {
                                handleIntent(SyllabusIntent.OnClassSelected(it))
    ...
    /*    bottomBar = {
        ...
        bottomBar = {
            ...
            BottomSearchBarView(
                ...
                placeholder = stringResource(R.string.feature_syllabus_search_placeholder),
        ...
        UiStateHandler(
            ...
            HorizontalPager(
                ...
                PullToRefreshBox(
                    ...
                    onRefresh = {handleIntent(SyllabusIntent.OnRefresh)}
                ) {
                    if (data.filteredSyllabuses.isEmpty()) {
                        EcareProEmptyState(
                            message = stringResource(R.string.feature_syllabus_empty_state)
                        )
                    } else {
                        LazyColumn(
                            modifier = Modifier
