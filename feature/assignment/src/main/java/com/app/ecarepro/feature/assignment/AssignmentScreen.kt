...
fun AssignmentScreen(
    ...
    openAttachments: (title: String, attachmentUrls: List<String>) -> Unit = { _, _ -> },
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.screenEvent.collect { event ->
            when (event) {
                ...
                is AssignmentEvent.ViewAttachments -> openAttachments(event.title, event.attachmentUrls)
    ...
    EcareProScaffold(
        ...
        UiStateHandler(
            ...
            if (data.filteredAssignments.isEmpty()) {
                EcareProEmptyState(
                    modifier = Modifier.padding(paddingValues),
                    message = "No results. Maybe try a broader search?"
                )
            } else {
                PullToRefreshBox(
                    modifier = Modifier
