...
class PopUpSubjectListAdapter(private var mySubjectList: List<MySubject>,
                              private var itemListener: ItemListener<MySubject>
) :
    RecyclerView.Adapter<PopUpSubjectListAdapter.PopUpListViewHolder>() {
    ...
    override fun getItemCount(): Int = mySubjectList.size

    override fun onBindViewHolder(holder: PopUpListViewHolder, pos: Int) {

        holder.itemName.text=mySubjectList[holder.bindingAdapterPosition].subjectName
        holder.llMain.setOnClickListener {
            lastIndex=holder.bindingAdapterPosition
             itemListener.onItemClick(mySubjectList[holder.bindingAdapterPosition],1,true)
