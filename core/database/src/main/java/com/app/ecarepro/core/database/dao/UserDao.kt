...
interface UserDao {
    ...
    suspend fun getUserByLocalId(localId: Int): UserEntity?

    @Query("SELECT * FROM users WHERE user_id = :userId AND user_type = :userType LIMIT 1")
    suspend fun getUserByUserIdAndType(userId: Int, userType: Int): UserEntity?

    @Query("DELETE FROM users WHERE id = :localId")
