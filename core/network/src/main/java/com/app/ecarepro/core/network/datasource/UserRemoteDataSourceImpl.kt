...
class UserRemoteDataSourceImpl @Inject constructor(
    ...
    override suspend fun applyLeave(request: NetworkApplyLeaveRequest): NetworkApplyLeaveResponse {
        return userService.applyLeave(request)
    }

    override suspend fun getLeaveSettings(): NetworkLeaveSettingResponse {
        return userService.getLeaveSettings()
    }

    override suspend fun generateToken(device: Int): String =
        userService.generateToken(device).unwrapPayload { tokenKey }

    override suspend fun logout(deviceID: String, sessionID: String) {
        userService.logout(deviceID = deviceID, sessionID = sessionID)
    }
}